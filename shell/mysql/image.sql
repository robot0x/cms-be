...
CREATE TABLE `image` (
  ...
  `type` varchar(10) NOT NULL DEFAULT '' COMMENT '图片的类型。''-未设置类型（没有被使用）/1-cover图-封面图/2-thumb图-缩略图/3-banner图/4-文章内容图。存储以逗号隔开的字符串，例如：1,2,4 即这张图片的类型为cover图 & thumb图 & 内容图', -- 12 封面图 缩略图
